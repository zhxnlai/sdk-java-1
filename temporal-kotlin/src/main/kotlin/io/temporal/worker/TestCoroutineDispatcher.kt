...
public class TestCoroutineDispatcher: CoroutineDispatcher(), Delay, DelayController {
    ...
    private val _time = AtomicLong(0L)
    ...
    private fun doActionsUntil(targetTime: Long) {
        while (true) {
            ...
            if (current.time != 0L) _time.set(current.time)
            current.run()
        }
    }

    /** @suppress */
    override val currentTime: Long get() = _time.get()
    ...
    private fun advanceUntilTime(targetTime: Long) {
        ...
        _time.getAndUpdate { currentValue -> max(currentValue, targetTime) }
    }
